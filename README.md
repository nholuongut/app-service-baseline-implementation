...
1. Obtain App gateway certificate
   ...
   - Set a variable for the domain that will be used in the rest of this deployment.
     ...
     ```bash
     export DOMAIN_NAME_APPSERV_BASELINE="luongutnho.com"
     ```
...
curl https://raw.githubusercontent.com/nholuongut/app-service-sample-workload/main/website/SimpleWebApp.zip -o SimpleWebApp.zip
...
1. Create an `A` record for DNS.
   ...
   Map the Azure Application Gateway public IP address to the application domain name. To do that, please edit your hosts file (`C:\Windows\System32\drivers\etc\hosts` or `/etc/hosts`) and add the following record to the end: `${APPGW_PUBLIC_IP} www.${DOMAIN_NAME_APPSERV_BASELINE}` (e.g. `50.140.130.120  www.luongutnho.com`)

1. Browse to the site (e.g. <https://www.luongutnho.com>).
